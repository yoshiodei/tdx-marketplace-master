...
<div class="mt-4 relative h-12">
	...
	{#if isOpen}
		<button
			...
			class="fixed block inset-0 w-full h-full z-100 bg-gray-200 opacity-50 cursor-default"
...
<!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
	<div
		...
		class:hidden={!isOpen}
		class="absolute left-0 w-full max-h-60 overflow-auto py-2 bg-white border-b-2 rounded shadow-md border-2 z-30 border-t-0 rounded-tl-none rounded-tr-none"
	...
	>
		{#each options.data as option (typeof option.value === 'object' ? option.value[options.idField] : option.value)}
			<div
				class="block px-3 py-1 rounded-none font-normal hover:text-white text-gray-800 hover:bg-primary-600"
			>
				<CheckboxControl
					tabindex="-1"
					checked={value &&
