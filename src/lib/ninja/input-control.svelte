<script>
	...
	import { createValidator, validate } from '$lib/ninja/utils/validator'
	import BinaryRecord from '$lib/ninja/page/binary-record.svelte'
...
<div class="mt-4 relative {type === 'textarea' ? 'sm:h-auto' : 'h-12'}">
	...
	{#if type === 'textarea'}
		<textarea
			rows="2"
			class="px-3 py-3 w-full min-h-[50px] h-12 shadow-none bg-white outline-none focus:ring-0 rounded z-10"
	...
	{:else if type === 'file'}
		<input
			class:hidden={value}
			bind:this={ref}
			class="px-3 py-3 w-full shadow-none bg-white outline-none focus:ring-0 rounded z-10"
			class:border={!focused}
			class:border-2={focused}
			class:border-gray-400={pristineOrValid}
			class:border-primary-600={focusedAndValidOrPristine}
		...
		{#if value}
			<div
				...
				class="absolute left-0 py-3 pl-3 text-xl w-full shadow-none bg-white border-gray-400 border rounded z-1"
			>
		...
		{#if ref && (ref.value || value)}
			<Button
				...
				classes="text-gray-800 hover:bg-gray-300 absolute right-0 pl-2 pr-0 pt-0 pb-1 mr-1 rounded-full"
				on:click={() => {
					ref.value = ''
...
<div class="flex flex-col mt-1 pr-3 text-xs text-red-600">
